...
class BraintreeHelper
{
    ...
    protected $_last_transaction_id;
    protected $_last_transaction_error;
    ...
    public function make_sale($paymentRequest)
    {
        $merchant_account_id = $this->_determine_merchant_account_from_currency($paymentRequest->currency);
        $configArray = ['amount' => $paymentRequest->amount];
        if ($this->_has_nonce($paymentRequest))
        {
            $configArray['merchantAccountId'] = $merchant_account_id;
            $configArray['paymentMethodNonce'] = $paymentRequest->payment_nonce;
            $result = $this->gateway->transaction()->sale($configArray);
        }
...
//        }
    ...
    private function _has_nonce($paymentRequest)
    {
        return (array_key_exists('payment_nonce', $paymentRequest) && !is_null($paymentRequest->payment_nonce));
    }
