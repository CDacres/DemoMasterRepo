...
// import { exampleSearchResults } from '../../../../stories/data';

// Connectors
import { useConfig, useSearch } from '@src/store/connectors';

// Utils
// import { convertLocationSlugToString, convertLocationStringToSlug, convertTagSlugToString } from '@src/utils';
...
class CombinedInput extends StatefulComponent<Props, State> {
  static defaultProps = {
  ...
  public inputName: string = 'searchInput';
  ...
  constructor(props: Props) {
    ...
    this.state = {
      ...
      value: '',
      // value: props.combinedValue ? convertLocationSlugToString(props.combinedValue) : '',
  ...
  shouldComponentUpdate(nextProps: Props, nextState: State) {
    const { combinedValue } = this.props;
    const { value } = this.state;
    if (nextProps.combinedValue !== combinedValue) {
      return true;
    }
  ...
  componentDidUpdate(prevProps: Props) {
    const { combinedValue } = this.props;
    if (prevProps.combinedValue !== combinedValue) {
      this.setState({ value: combinedValue });
      // this.setState({ value: convertLocationSlugToString(combinedValue) });
  ...
  handleFocus = () => {
    const { handleContainerFocus, combinedValue } = this.props;
    if (combinedValue !== '') {
      // this.handleGooglePlaces(combinedValue);
    } else {
  ...
  //   const { handleContainerBlur, selectValue } = this.props;
  //   this.unstable = true;
  //   const latLon = await this.handleGeocoding(description);
  //   selectValue(place_id, convertLocationStringToSlug(description), latLon);
  //   this.goToState('default');
...
export default useConfig(useSearch(CombinedInput)) as any; // the horror, the horror
