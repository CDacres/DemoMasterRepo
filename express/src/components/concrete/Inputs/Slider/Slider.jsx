...
import { HORIZONTAL, VERTICAL, PERCENT_FULL, PERCENT_EMPTY, KEYS } from './constants/SliderConstants';
...
class Slider extends React.Component {
  ...
  getProgressStyle(idx) {
    ...
    if (idx === 0) {
      return orientation === VERTICAL ? { height: `${value}%`, top: '0px' } : { left: '0px', width: `${value}%` };
    }

    const prevValue = handlePos[idx - 1];
    const diffValue = value - prevValue;

    return orientation === VERTICAL ? { height: `${diffValue}%`, top: `${prevValue}%` } : { left: `${prevValue}%`, width: `${diffValue}%` };
  }
  ...
  getHandleDimensions() {
    ...
    return orientation === VERTICAL ? this.handleNode.clientHeight : this.handleNode.clientWidth;
  }
  ...
  handleClick(ev) {
    ...
    const { onClick, orientation } = this.props;

    // Calculate the position of the slider on the page so we can determine
    // the position where you click in relativity.
    const sliderBox = this.getSliderBoundingBox();

    const positionDecimal = orientation === VERTICAL ? (ev.clientY - sliderBox.top) / sliderBox.height : (ev.clientX - sliderBox.left) / sliderBox.width;
  ...
  userAdjustPosition(idx, proposedPosition) {
    ...
    if (getNextHandlePosition) {
      ...
      if (Number.isNaN(nextPosition) || nextPosition < PERCENT_EMPTY || nextPosition > PERCENT_FULL) {
  ...
  validatePosition(idx, proposedPosition) {
    ...
    const handlePercentage = orientation === VERTICAL ? ((handleDimensions / sliderBox.height) * PERCENT_FULL) / 2 : ((handleDimensions / sliderBox.width) * PERCENT_FULL) / 2;

    return Math.max(
      Math.min(
        nextPosition,
        handlePos[idx + 1] !== undefined ? handlePos[idx + 1] - handlePercentage : PERCENT_FULL, // 100% is the highest value
      ),
      handlePos[idx - 1] !== undefined ? handlePos[idx - 1] + handlePercentage : PERCENT_EMPTY, // 0% is the lowest value
    );
  }
  ...
  canMove(idx, proposedPosition) {
    ...
    const nextHandlePosition = handlePos[idx + 1] !== undefined ? handlePos[idx + 1] - handlePercentage : Infinity;

    if (proposedPosition > nextHandlePosition) {
      return false;
    }

    const prevHandlePosition = handlePos[idx - 1] !== undefined ? handlePos[idx - 1] + handlePercentage : -Infinity;
  ...
  render() {
    const { autoAdjustVerticalPosition, algorithm, children, css, disabled, handle: Handle, max, min, orientation, pitComponent, pitPoints, background: Background, progressBar: ProgressBar, styles } = this.props;
    ...
    return (
      ...
      <div
        ...
        {...css(autoAdjustVerticalPosition && styles.autoAdjustVerticalPosition, orientation === VERTICAL && styles.rheostat__vertical)}
      >
          ...
          <div {...css(styles.rheostat)}>
            ...
            >
              {handlePos.map((pos, idx) => {
                const handleStyle = orientation === VERTICAL ? { top: `${pos}%`, position: 'absolute' } : { left: `${pos}%`, position: 'absolute' };
