...
class ICal
{
    ...
    public function convert_to_local_time($dateTimeStr, $timeZoneId, $originalTimeZone="UTC")
    {
        $dateTime=new DateTime($dateTimeStr,new DateTimeZone($originalTimeZone));
        if ($originalTimeZone!==$timeZoneId)
        {
    ...
    public function process_recurrences()
    {
        ...
        if (empty($array['VEVENT']))
            return false;
        if (!isset($this->cal['VTIMEZONE']) || !isset($this->cal['VTIMEZONE']['TZID']))
        {
            $this->cal['VTIMEZONE']['TZID']="UTC";
        }
        foreach ($array['VEVENT'] as $anEvent)
        {
            if (!isset($anEvent['DTSTART_array'][0]['TZID']))
            {
                $anEvent['DTSTART']=$this->convert_to_local_time($anEvent['DTSTART'],$this->cal['VTIMEZONE']['TZID']);
                $anEvent['DTEND']=$this->convert_to_local_time($anEvent['DTEND'],$this->cal['VTIMEZONE']['TZID']);
            }
            elseif ($anEvent['DTSTART_array'][0]['TZID']!==$this->cal['VTIMEZONE']['TZID'])
            {
                $anEvent['DTSTART']=$this->convert_to_local_time($anEvent['DTSTART'],$this->cal['VTIMEZONE']['TZID'],$anEvent['DTSTART_array'][0]['TZID']);
                $anEvent['DTEND']=$this->convert_to_local_time($anEvent['DTEND'],$this->cal['VTIMEZONE']['TZID'],$anEvent['DTSTART_array'][0]['TZID']);
            }
            if (isset($anEvent['EXRULE']))
            {
                throw new Exception("Repeat rule 'exceptions by rule' are currently not supported. Please contact the Zipcube tech team quoting this message.");
            }
            if (isset($anEvent['RRULE']) && $anEvent['RRULE'] != '') {
                ...
                else
                {
                    $newExDates=array();
                    $timezone=null;
                    reset($anEvent['EXDATE_array']);
                    do
                    {
                        $item=current($anEvent['EXDATE_array']);
                        if (is_array($item))
                        {
                            if (isset($item['TZID']))
                            {
                        ...
                        {
                            $newExDates[]=$this->convert_to_local_time($item,$this->cal['VTIMEZONE']['TZID'],$timezone);
                            $timezone=null;
                        }
                    }
                    while(next($anEvent['EXDATE_array'])!==false);
                    $anEvent['EXDATE_array']=$newExDates;
                }
        ...
        $this->cal['VEVENT'] = array_merge($parsedEvents,$calculatedEvents);
    }
    /**
